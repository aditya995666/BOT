...
with col1:
    if st.button("Send Direct Command", use_container_width=True):
        if not direct_input.strip():
            ...
            try:
                # 🔥 FIX: OS handler pehle check karo (kyunki router already OS keywords handle karta hai)
# OS handler direct call karo for all commands, router decide karega intent
