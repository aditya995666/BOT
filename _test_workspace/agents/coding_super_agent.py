...
class CodingSuperAgent:
    ...
    # Improve code using previous issues
            ...
            validation_result = self._self_validate(initial_code, query)
...
"""
    ...
    def _extract_code_block(self, text: str) -> str:
        """Extract code block from Gemini response - ULTIMATE VERSION"""
        if not text:
            return ""
        ...
        # Method 3: Find function/class definitions without backticks
        ...
        for line in lines:
            ...
            # Start collecting when we see code keywords
            if not started:
                if (line_stripped.startswith(('def ', 'class ', 'import ', 'from ')) or
                    line_stripped.startswith(('print(', 'return ')) or
                    '=' in line_stripped and len(line_stripped) < 100):
                    started = True
                    code_lines.append(line)
                continue
            
            # Stop at explanation markers
            if any(x in line.lower() for x in ['explanation', 'output:', 'result:', 
                                            'example:', 'note:', 'here\'s how', 
                                            'this code will', 'the code', 'this function']):
                break
            
            code_lines.append(line)
        
        if code_lines:
            print(f"✅ Extracted {len(code_lines)} lines from context")
            return '\n'.join(code_lines)
        
        # Method 4: Look for any code-like patterns
        code_candidates = []
        for line in lines:
        ...
        if code_candidates:
            print(f"✅ Found {len(code_candidates)} code-like lines")
            return '\n'.join(code_candidates[:20])  # First 20 lines
